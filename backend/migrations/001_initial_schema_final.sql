-- 删除现有表（如果存在�?DROP TABLE IF EXISTS user_favorites CASCADE;
...
-- 创建访客会话�?CREATE TABLE guest_sessions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- 创建用户活动�?CREATE TABLE user_activities (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- 创建用户收藏�?CREATE TABLE user_favorites (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES users(id),
...
-- 插入默认超级管理�?INSERT INTO users (username, email, password_hash, role, is_v_ip, language, timezone)
